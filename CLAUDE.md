...
**osubot-plugin**：`lib.rs`（PluginManager — WASM 插件加载/调度/epoch 超时隔离/生命周期管理/热重载 diff）、`bridge.rs`（HostServices + `host_call_impl` 七种宿主函数分发 + SSRF 防护）、`instance.rs`（PluginInstance — wasmtime 实例封装 + 内存协议）、`config.rs`（插件配置 TOML 反序列化）。插件开发文档见 `docs/plugin-development.md`。
