...
pub use cache::{cleanup_expired, ensure_cache_dir, fetch_image_data_uri};
...
pub async fn render_score_panel_html(
    html: &str,
    width: u32,
    height: u32,
) -> Result<Vec<u8>, RenderError> {
    let html_with_inlined = cache::inline_external_images(html).await;
    let _permit = render_semaphore()
        .acquire()
        .await
        .expect("render semaphore never closed");
    let wrapped_html = score_panel_html::style::wrap_score_panel_css(&html_with_inlined);
    let font_ctx = get_font_context();
    let cancel = Arc::new(std::sync::atomic::AtomicBool::new(false));
    let cancel_flag = cancel.clone();
    let render_result = tokio::time::timeout(
