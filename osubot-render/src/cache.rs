...
fn find_src_url(bytes: &[u8], tag_start: usize) -> Option<(usize, usize)> {
    ...
    let after_src = &search_window[src_pos + 3..];
    let eq_pos = after_src.iter().position(|&b| b == b'=')?;
    let after_eq = &after_src[eq_pos + 1..];
    let quote_start = after_eq.iter().position(|&b| b == b'"' || b == b'\'')?;
    let quote_char = after_eq[quote_start];
    let value_start = tag_start + src_pos + 3 + eq_pos + 1 + quote_start + 1;
    let quote_pos = bytes[value_start..].iter().position(|&b| b == quote_char)?;
...
pub async fn inline_external_images(html: &str) -> String {
    ...
    for r in refs.iter().rev() {
        let replacement = url_to_data
            .get(&r.url)
            .map(|s| s.as_str())
            .unwrap_or(TRANSPARENT_GIF);
        result.splice(r.start..r.end, replacement.bytes());
    }
...
mod tests {
    ...
    #[test]
    fn test_extract_image_refs_spaces_around_equals() {
        let html = r#"<img src = "https://a.ppy.sh/1.png">"#;
        let refs = extract_image_refs(html);
        assert_eq!(refs.len(), 1);
        assert_eq!(refs[0].url, "https://a.ppy.sh/1.png");
    ...
    #[test]
    fn test_extract_image_refs_single_quotes() {
        let html = r#"<img src='https://a.ppy.sh/1.png'>"#;
        let refs = extract_image_refs(html);
        assert_eq!(refs.len(), 1);
        assert_eq!(refs[0].url, "https://a.ppy.sh/1.png");
    ...
    #[test]
    fn test_inline_external_images_replaces_failed_with_placeholder() {
        let rt = tokio::runtime::Runtime::new().unwrap();
        let html = r#"<img src="https://invalid.example.com/404.png">"#;
        let result = rt.block_on(inline_external_images(html));
        assert!(
            !result.contains("https://invalid.example.com"),
            "external URL should be replaced: {}",
            result
        );
        assert!(
            result.contains("data:image/gif;base64,"),
            "should contain data URI placeholder: {}",
            result
        );
    }
