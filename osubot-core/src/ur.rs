...
fn replay_cache_dir() -> std::path::PathBuf {
    dirs::cache_dir()
...
async fn download_replay(
    ...
    let cache_path = replay_cache_dir().join(format!("{}.osr", score_id));
    ...
    let bytes = crate::api::retry_on_401(oauth, 3, || async {
        ...
        if !resp.status().is_success() {
            tracing::warn!(url = %url, status = %resp.status(), "Replay download failed with non-success status");
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let _ = std::fs::write(&write_path, &bytes_clone);
    })
    .await
    .ok();
...
fn parse_replay_key_times(osr_bytes: &[u8]) -> Option<(Vec<f64>, f64)> {
    ...
    for event in replay.replay_data.iter() {
        ...
        if first_frame {
            ...
            if !(0..=10000).contains(&delta) {
                ...
                tracing::debug!(
                    delta,
                    time_offset,
                    "Skipping first frame, using as time offset"
                );
                continue;
            }
        }
...
pub async fn calculate_score_ur(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        if timing_errors.len() >= 5 {
            ...
            tracing::debug!(score_id, first_errors = ?first_5, "First 5 timing errors");
            let mean: f64 =
            ...
            tracing::debug!(score_id, mean, std_dev, "Timing error statistics");
        }
