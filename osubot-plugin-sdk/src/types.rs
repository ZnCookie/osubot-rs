...
pub struct Command {
    ...
    ///
    /// 行为：
    /// - 对于模式敏感命令（`~` / `where` / `!p` / `!r` / `!s` / `!ps` / `!rs` / `!ss` / `今日高光` / `!a`）：取用户在命令中显式指定的模式；若未指定，则取该用户的默认模式，未设置时回退到 `Osu`。最终永远为 `Some(...)`。
    /// - 对于 `!rv`：取用户在命令中显式指定的模式（`!rv :1`）；若未指定，则为 `None`（不会回退到用户默认模式，而是使用谱面原始模式）。
    /// - 对于其他命令（`绑定` / `解绑` / `!profile` / `!help` / `!mode`）：`None`，不代表任何用户输入。
    ///
