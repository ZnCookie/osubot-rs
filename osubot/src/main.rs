                ...
                Ok(profile) => {
                    ...
                    .await
                    {
                        Ok(jpeg_bytes) => {
                            ...
                            if send_group_msg_with_image(&ctx.write, msg.group_id, &jpeg_bytes)
                                .await
                                .is_err()
                            {
...
async fn send_group_msg_with_image(
    ...
    let estimated_size = (image_data.len() * 4 / 3) + 256;
    if estimated_size > MAX_MESSAGE_BYTES {
        warn!(
            group_id = group_id,
            image_len = image_data.len(),
            "Image too large for OneBot message"
        );
        return Err(());
    }
